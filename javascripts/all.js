...
(function() {
  var ValidationContext, Validations,
    __slice = [].slice;

  ValidationContext = (function() {

    function ValidationContext(subject) {
      this.subject = subject;
    }

    ValidationContext.prototype.getDsl = function(source) {
      var dsl, func, me, name;
      if (source == null) {
        source = ko.Validations.validators;
      }
      dsl = {};
      me = this;
      for (name in source) {
        func = source[name];
        this.wrapValidator(dsl, name, func);
      }
      return dsl;
    };

    ValidationContext.prototype.wrapValidator = function(dsl, name, func) {
      var me;
      me = this;
      return dsl[name] = function() {
        var field, fields, options, _i, _j, _len;
        fields = 2 <= arguments.length ? __slice.call(arguments, 0, _i = arguments.length - 1) : (_i = 0, []), options = arguments[_i++];
        ...
        for (_j = 0, _len = fields.length; _j < _len; _j++) {
          field = fields[_j];
          me.setValidator(func, field, options);
        }
        return dsl;
      };
    };

    ValidationContext.prototype.setValidator = function(validator, field, options) {
      var me, validatorSubscriber, _base;
      me = this;
      me._validations || (me._validations = {});
      (_base = me._validations)[field] || (_base[field] = []);
      validatorSubscriber = ko.dependentObservable(function() {
        var allowedByOnly, deniedByExcept, except, only, shouldValidate;
        only = options.only, except = options.except;
        allowedByOnly = !only || only.call(me.subject);
        deniedByExcept = except && except.call(me.subject);
        shouldValidate = allowedByOnly && !deniedByExcept;
        if (shouldValidate) {
          return validator.call(me, me.subject, field, options);
        }
      });
      validatorSubscriber.subscribe(function(newError) {
        var actualError, currentError;
        currentError = me.subject.errors[field];
        actualError = [currentError(), newError].exclude(function(x) {
          return !x;
        }).join(", ");
        return me.subject.errors[field](actualError || null);
      });
      if (me._validations[field].isEmpty()) {
        validatorSubscriber.subscribe(function() {
          return me.subject.errors[field](null);
        }, me.subject, "beforeChange");
      }
  ...
  Validations = {
    ClassMethods: {
      extended: function() {
        return this.include(Validations.InstanceMethods);
      }
    },
    InstanceMethods: {
      isValid: function() {
      ...
      enableValidations: function() {
        var dsl;
        if (!this.constructor.validates) {
          return false;
        }
        this.validationContext = new ValidationContext(this);
        dsl = this.validationContext.getDsl();
        this.constructor.validates.call(dsl, this);
        return true;
      }
    }
  };
...
(function() {
  ...
  ko.Validations.validators = {
    acceptance: function(model, field, options) {
      var val;
      val = model[field]();
      if (!val) {
        return options.message || "needs to be accepted";
      } else {
        return null;
      }
    },
    presence: function(model, field, options) {
      var isBlank, val;
      val = model[field]();
      isBlank = !val || val.toString().isBlank();
      if (isBlank) {
    ...
    email: function(model, field, options) {
      var isValid, val;
      val = model[field]();
      isValid = !val || val.toString().match(/.+@.+\..+/);
      if (!isValid) {
        return options.message || "should be a valid email";
      } else {
        return null;
      }
    },
    confirmation: function(model, field, options) {
      var orig, other, otherField;
      otherField = options.confirms;
      if (!otherField) {
        throw "Please specify which field to apply the confirmation to using {confirms: 'otherField'}";
      }
      orig = model[field]();
      other = model[otherField]();
      if (orig !== other && orig) {
        return options.message || ("should confirm " + otherField);
      } else {
        return null;
      }
    },
    numericality: function(model, field, options) {
      var looksLikeNumeric, max, min, num, val;
      val = model[field]();
      if (!val) {
        return;
      }
      looksLikeNumeric = val.toString().match(/^-?\d+$/);
      num = parseInt(val, 10);
      min = options.min != null ? options.min : num;
      max = options.max != null ? options.max : num;
      if (looksLikeNumeric && (min <= num && num <= max)) {
        return null;
      } else {
        return options.message || "should be numeric";
      }
    },
    inclusion: function(model, field, options) {
      var val, values;
      values = options.values;
      if (!values) {
        throw "Please specify the values {values: [1, 2, 5]}";
      }
      val = model[field]();
      if (!val) {
        return;
      ...
      if (values.indexOf(val) < 0) {
        return options.message || ("should be one of " + (values.join(', ')));
      } else {
        return null;
      }
    },
    exclusion: function(model, field, options) {
      var val, values;
      values = options.values;
      if (!values) {
        throw "Please specify the values {values: [1, 2, 5]}";
      ...
      if (values.indexOf(val) >= 0) {
        return options.message || ("should not be any of " + (values.join(', ')));
      } else {
        return null;
      }
    },
    format: function(model, field, options) {
      var matcher, val;
      matcher = options.match;
      if (!matcher) {
        throw "Please specify the match RegEx {match: /\d+/}";
      }
      val = model[field]();
      if (!val) {
        return;
      }
      if (val.toString().match(matcher)) {
        return null;
      } else {
        return options.message || "should be formatted properly";
      }
    },
    length: function(model, field, options) {
      var createMsg, max, min, val;
      val = model[field]();
      if (!val) {
        return;
      }
      ...
      createMsg = function() {
        var maxMsg, minMsg, separator;
        minMsg = options.min != null ? "at least " + min + " characters long" : "";
        maxMsg = options.max != null ? "no longer than " + max + " characters" : "";
        separator = minMsg && maxMsg ? " but " : "";
        return "should be " + minMsg + separator + maxMsg;
      };
      if ((min <= val && val <= max)) {
        return null;
      } else {
        return options.message || createMsg();
      }
    },
    custom: function(model, field, options) {
      return options.call(model, model);
    }
  };

}).call(this);
(function() {
  var Ajax, Callbacks, Events, Model, Module, moduleKeywords,
  ...
  Events = {
    ClassMethods: {
      extended: function() {
        ...
        return this.include(Events.InstanceMethods);
      },
      upon: function(eventName, callback) {
        var _base;
    ...
    InstanceMethods: {
      trigger: function() {
        var args, callback, eventName, events, handlers, _i, _len;
  ...
  Callbacks = {
    ClassMethods: {
      beforeSave: function(callback) {
        return this.upon('beforeSave', callback);
      }
    }
  };
  ...
  Ajax = {
    ClassMethods: {
      persistAt: function(className) {
        this.className = className;
        return this.getUrl || (this.getUrl = function(model) {
          var collectionUrl;
          if (model && model.getUrl) {
            return model.getUrl(model);
          }
          collectionUrl = "/" + (className.toLowerCase()) + "s";
          if (model != null ? model.id() : void 0) {
            collectionUrl += "/" + (model.id());
          }
          return collectionUrl;
        });
      },
      extended: function() {
        return this.include(Ajax.InstanceMethods);
      }
    },
    InstanceMethods: {
      ignore: function() {
      ...
      mapping: function() {
        var k, mappable, v;
        if (this.__ko_mapping__) {
          return this.__ko_mapping__;
        }
        mappable = {
          ignore: this.ignore()
        };
        for (k in this) {
          ...
          if (!ko.isObservable(v)) {
            mappable.ignore.push(k);
          }
        }
        return this.__ko_mapping__ = mappable;
      },
      toJSON: function() {
        return ko.mapping.toJS(this, this.mapping());
      },
      save: function() {
        var allowSaving, data, params;
        allowSaving = this.isValid();
        if (!allowSaving) {
          return false;
        }
        this.trigger('beforeSave');
        data = {};
        data[this.constructor.className] = this.toJSON();
        params = {
          type: this.persisted() ? 'PUT' : 'POST',
          dataType: 'json',
          ...
          url: this.constructor.getUrl(this),
          contentType: 'application/json',
          context: this,
          processData: false,
          data: JSON.stringify(data),
          statusCode: {
            422: function(xhr, status, errorThrown) {
              var errorData;
              ...
              return this.updateErrors(errorData);
            }
          }
        };
        return $.ajax(params).done(function(resp, status, xhr) {
  ...
  Model = (function(_super) {
    ...
    Model.extend(Ajax.ClassMethods);

    Model.extend(Events.ClassMethods);

    Model.extend(Callbacks.ClassMethods);

    Model.extend(ko.Validations.ClassMethods);

    Model.fields = function() {
      var fieldNames;
      fieldNames = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
      fieldNames = fieldNames.flatten();
    ...
    function Model(json) {
      ...
      me = this;
      this.set(json);
      this.id || (this.id = ko.observable());
      this.mapping().ignore.exclude('constructor').filter(function(v) {
        return !v.startsWith('_') && Object.isFunction(me[v]);
      }).forEach(function(fn) {
        var original;
        original = me[fn];
        me[fn] = original.bind(me);
        me._originals || (me._originals = {});
        return me._originals[fn] = original;
      });
      this.persisted = ko.dependentObservable(function() {
        return !!me.id();
      });
    }

    Model.prototype.set = function(json) {
      var availableFields, ignores, key, me, _base, _i, _len;
      ko.mapping.fromJS(json, this.mapping(), this);
      me = this;
      this.errors || (this.errors = {});
      ignores = this.mapping().ignore;
      availableFields = this.constructor.fieldNames;
      availableFields || (availableFields = this.constructor.fields(Object.keys(json)));
      for (_i = 0, _len = availableFields.length; _i < _len; _i++) {
        key = availableFields[_i];
        if (!(ignores.indexOf(key) < 0)) {
          continue;
        }
        this[key] || (this[key] = ko.observable());
        (_base = this.errors)[key] || (_base[key] = ko.observable());
      }
      this.enableValidations();
      return this;
    };

    Model.prototype.updateErrors = function(errorData) {
      var error, field, key, message, setter, _ref;
      _ref = this.errors;
      for (key in _ref) {
        setter = _ref[key];
        field = this.errors[key];
        error = errorData[key];
        message = error && error.join ? error.join(", ") : error;
        if (field) {
          setter(message);
        }
      }
